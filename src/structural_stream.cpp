...
template <typename T>
void json_array(std::ostringstream& out, const std::vector<T>& values) {
    out << '[';
...
std::string StructuralEvent::to_json() const {
    std::ostringstream out;
    out << "{\"version\":" << version
        << ",\"source_id\":\"" << escape_json(source_id) << "\""
        << ",\"sequence\":" << sequence
        << ",\"byte_offset\":" << byte_offset
        << ",\"byte_length\":" << byte_length
        << ",\"trail\":";
    json_array(out, trail);
    out << ",\"relation_ids\":";
    json_array(out, relation_ids);
...
StructuralEvent StructuralExtractor::extract(const std::vector<std::uint8_t>& bytes,
    ...
    event.byte_length = bytes.size();
    event.trail = encoded.trail;
    event.resolution = max_layers_;
    for (const auto id : event.trail) {
        if (id >= 256) event.relation_ids.push_back(id);
    }
...
StructuralStream::StructuralStream(StructuralExtractor& extractor, std::size_t window_size,
    ...
    buffer_.reserve(window_size_ + hop_size_);
}
...
std::vector<StructuralEvent> StructuralStream::emit_ready(const std::string& source_id) {
    std::vector<StructuralEvent> events;
    while (buffer_.size() >= window_size_) {
        std::vector<std::uint8_t> window(buffer_.begin(), buffer_.begin() + window_size_);
        events.push_back(extractor_.extract(window, source_id, sequence_++, buffer_offset_));
...
std::vector<StructuralEvent> StructuralStream::flush(const std::string& source_id) {
    auto events = emit_ready(source_id);
    if (!buffer_.empty()) {
        std::vector<std::uint8_t> tail = buffer_;
        events.push_back(extractor_.extract(tail, source_id, sequence_++, buffer_offset_));
