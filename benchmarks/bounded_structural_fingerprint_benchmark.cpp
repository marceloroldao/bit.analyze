...
int main(){const auto base=base_data();const auto ref_events=events_for(base,17);const auto exact_ref=make_structural_fingerprint(ref_events,base.size(),8);std::cout<<"bounded_structural_fingerprint_v1_capacity_sweep\ncapacity,mutation_fraction,mutated_bytes,v0_combined,v1_combined,absolute_error,chunk_invariant,storage_bytes\n"<<std::fixed<<std::setprecision(6);for(std::size_t cap:{64u,128u,256u,512u,1024u,2048u,4096u}){BoundedStructuralFingerprintConfig cfg;cfg.frequency_buckets=cap;cfg.transition_buckets=cap;cfg.regions=8;const auto bounded_ref=make_bounded_structural_fingerprint(ref_events,base.size(),cfg);for(double f:{0.0,1.0/double(base.size()),0.001,0.01,0.05,0.10,0.25,0.50}){auto d=mutate(base,f);auto e17=events_for(d,17),e257=events_for(d,257);auto exact=make_structural_fingerprint(e17,d.size(),8);auto a=make_bounded_structural_fingerprint(e17,d.size(),cfg),b=make_bounded_structural_fingerprint(e257,d.size(),cfg);auto v0=compare_structural_fingerprints(exact_ref,exact).combined;auto v1=compare_bounded_structural_fingerprints(bounded_ref,a).combined;auto changed=f==0.0?0:static_cast<std::size_t>(std::ceil(base.size()*f));std::cout<<cap<<','<<f<<','<<changed<<','<<v0<<','<<v1<<','<<std::abs(v0-v1)<<','<<(equivalent(a,b)?1:0)<<','<<a.storage_bytes()<<'\n';}}}
