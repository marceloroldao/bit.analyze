...
int main(){auto a=base(65536);auto f0=run(a);const std::size_t steps[]={0,256,64,16,4};double first_changed=-1, last=-1;std::cout<<"structural_change_gradient_v0\nmutation_every,regional_distance\n";for(auto e:steps){auto f=run(mutate(a,e));double d=distance(f0.regional_frequency,f.regional_frequency);std::cout<<e<<","<<d<<"\n";if(e==256)first_changed=d;last=d;}return first_changed>0&&last>0?0:1;}
